...
const PricingPage: React.FC = () => {
  ...
  const daycarePlans = MOCK_PRICING_PLANS.filter(p => p.role === UserRole.FOUNDATION);
  const supplierPlan = MOCK_PRICING_PLANS.find(p => p.role === UserRole.PRODUCT_SUPPLIER);
  const serviceProviderPlan = MOCK_PRICING_PLANS.find(p => p.role === UserRole.SERVICE_PROVIDER);
    ...
    <div className="bg-page-bg min-h-screen py-12 px-4 sm:px-6 lg:px-8 relative text-swiss-charcoal">
      ...
      <div className="max-w-7xl mx-auto pt-16 sm:pt-12">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-swiss-charcoal">{fromSignup ? t('pricingPage.titleSignup') : 'ProCrèche Solutions – Pricing Guide'}</h1>
        ...
        {shouldShowSuppliersAndProviders && (
             <div className="mt-16">
                <h2 className="text-3xl font-semibold text-center text-swiss-charcoal">ProCrèche Solutions – Pricing for Suppliers & Service Providers</h2>

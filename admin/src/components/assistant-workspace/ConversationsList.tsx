...
/**
 * AI conversations section rendered beneath the existing admin sidebar nav
 * (additive only — the nav itself is strategy-locked). ADMIN/SUPER_ADMIN +
 * v2_admin_assistant flag only.
 */
export const ConversationsList: React.FC<ConversationsListProps> = ({ onNavigate }) => {
  ...
  const { enabled } = useFeatureFlag('v2_admin_assistant');
    ...
    <div className="mt-4 border-t border-gray-200/80 pt-3">
      ...
      {groups.map((group) => (
        <div key={group.key} className="mb-1.5">
          <p className="px-4 py-1 text-[11px] text-gray-400">{groupLabels[group.key]}</p>
          {group.items.map((conversation) => {
            ...
            return (
              <div
                key={conversation.id}
                className={`group flex items-center gap-2 rounded-button px-4 py-1.5 text-xs transition-colors ${
                  isActive

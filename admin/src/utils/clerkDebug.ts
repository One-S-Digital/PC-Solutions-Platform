...
export function logClerkDiagnostics(publishableKey?: string): void {
  ...
  console.group('🔍 CLERK DIAGNOSTICS REPORT [ADMIN]');
  ...
  (window as any).__CLERK_DIAGNOSTICS_ADMIN__ = diagnostics;
  console.log('💡 TIP: Diagnostics saved to window.__CLERK_DIAGNOSTICS_ADMIN__');
}
...
export function monitorClerkRequests(): void {
  ...
  window.fetch = async (...args) => {
    ...
    if (url.includes('clerk.') || url.includes('clerk-')) {
      console.group(`🌐 [ADMIN] Clerk Network Request: ${url}`);
  ...
  console.log('✅ [ADMIN] Clerk request monitoring enabled');
}

// Export for use in browser console
if (typeof window !== 'undefined') {
  (window as any).debugClerkAdmin = {
    getDiagnostics: getClerkDiagnostics,
    logDiagnostics: logClerkDiagnostics,
    monitorRequests: monitorClerkRequests,
  };
  console.log('💡 TIP: Use window.debugClerkAdmin.logDiagnostics() to run diagnostics');
}

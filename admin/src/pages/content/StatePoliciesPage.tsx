...
import ContentUploadModal, { COUNTRIES_FOR_POLICIES, REGIONS_BY_COUNTRY } from '../../components/ContentUploadModal'
...
export default function StatePoliciesPage() {
  ...
  // Keep region valid when country changes
  useEffect(() => {
    if (!region) return
  ...
  const fetchContent = useCallback(
    async (page = pagination.page) => {
      ...
      try {
        ...
        const res = await api.getStatePolicies(apiClient, {
          page,
        ...
        if (res.data?.success) {
          ...
          throw new Error(res.data?.message || 'Failed to fetch state policies')
        }
      } catch (e: any) {
        toast.error(e?.response?.data?.message || e?.message || 'Failed to fetch state policies')
      } finally {
        setIsLoading(false)
      }
    },
  ...
  }, [debouncedSearch, country, region, status, criticalOnly, i18n.language])
  ...
  const handleSubmit = async (data: any, file?: File, onProgress?: (progress: number) => void) => {
    try {
      ...
      if (editingContent) {
        ...
        const uploadPromise = async () => api.uploadStatePolicy(apiClient, formData, onProgress)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await api.deleteStatePolicy(apiClient, id)
      if (res.data?.success) {
  ...
  const emptyLabel = useMemo(() => {
    if (search.trim()) return t('admin:content.statePolicies.noResults', 'No results found')
    return t('admin:content.statePolicies.empty', 'No state policies yet')
  }, [search, t])
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {t('admin:content.statePolicies.total', 'Total')}: {pagination.total}
              </p>
            </div>
          </div>
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          <div className="lg:col-span-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('admin:content.statePolicies.searchPlaceholder', 'Search policies...')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('admin:content.country', 'Country')}
            </label>
            <select
              value={country}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('admin:content.state', 'State/Region')}
            </label>
            <select
              value={region}
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            >
              <option value="">{t('admin:content.allStates', 'All')}</option>
              {regionOptions.map((r) => (
          ...
          <div className="flex items-end">
            <label className="inline-flex items-center gap-2 text-sm text-gray-700 select-none">
              ...
              />
              {t('admin:content.statePolicies.criticalOnly', 'Critical only')}
            </label>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-sm text-gray-500">{t('admin:content.statePolicies.loading', 'Loading...')}</p>
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          ...
          >
            ...
            {t('admin:content.statePolicies.addFirst', 'Add your first item')}
          </button>
        </div>
      ) : (
        <>

...
import { AcademicCapIcon, MagnifyingGlassIcon, PlusIcon } from '@heroicons/react/24/outline'
import { toast } from 'sonner'
import { useTranslation } from 'react-i18next'
import i18n from '../../i18n'
import ContentUploadModal, { ELEARNING_CATEGORIES } from '../../components/ContentUploadModal'
...
export default function ELearningContentPage() {
  ...
  const fetchContent = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        const res = await api.getELearning(apiClient, {
          page,
        ...
        if (res.data?.success) {
          ...
          throw new Error(res.data?.message || 'Failed to fetch e-learning content')
        }
      } catch (e: any) {
        toast.error(e?.response?.data?.message || e?.message || 'Failed to fetch e-learning content')
      } finally {
        setIsLoading(false)
      }
    },
  ...
  const handleSubmit = async (data: any, file?: File, onProgress?: (progress: number) => void) => {
    try {
      ...
      if (editingContent) {
        ...
        const uploadPromise = async () => api.uploadELearning(apiClient, formData, onProgress)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await api.deleteELearning(apiClient, id)
      if (res.data?.success) {
  ...
  const emptyLabel = useMemo(() => {
    if (search.trim()) return t('admin:content.eLearning.noResults', 'No results found')
    return t('admin:content.eLearning.empty', 'No e-learning content yet')
  }, [search, t])
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">{t('admin:content.eLearning.title', 'E-Learning')}</h2>
              <p className="text-sm text-gray-600">
                {t('admin:content.eLearning.total', 'Total')}: {pagination.total}
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('admin:content.eLearning.searchPlaceholder', 'Search e-learning...')}
      ...
      {isLoading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-sm text-gray-500">{t('admin:content.eLearning.loading', 'Loading...')}</p>
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          ...
          >
            ...
            {t('admin:content.eLearning.addFirst', 'Add your first item')}
          </button>
        </div>
      ) : (
        <>

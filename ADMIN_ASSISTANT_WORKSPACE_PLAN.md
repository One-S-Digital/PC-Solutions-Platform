...
- **Dashboard entry point (done):** the classic Dashboard (`admin/src/pages/Dashboard.tsx`) has an "AI Assistant" CTA banner in its Quick Actions card — it opens the floating panel via `AssistantContext`. When Phase B ships, this entry point is upgraded to navigate to `/assistant` workspace instead. The `AssistantContext` (`admin/src/contexts/AssistantContext.tsx`) was introduced to lift open-state out of `AssistantContainer` so that any page can trigger the panel.

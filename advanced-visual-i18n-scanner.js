...
class AdvancedVisualI18nScanner {
  constructor(options = {}) {
    this.browser = null;
    this.page = null;
    ...
    this.scannedPages = [];
    this.baseUrl = options.baseUrl || process.env.FRONTEND_URL || 'http://localhost:3000';
  ...
  async init() {
    console.log('🚀 Starting Advanced Visual i18n Scanner...');
  ...
  async handleAuthentication() {
    if (this.auth.type === 'clerk') {
      // For Clerk authentication, we'll need to handle it manually
      console.log('🔐 Handling Clerk authentication...');
      // This would need to be customized based on your auth flow
    }
  }
  ...
  async generateReport() {
    console.log('\n📊 Generating Advanced Visual i18n Report...');
    ...
    fs.writeFileSync('./advanced-visual-i18n-report.json', JSON.stringify(report, null, 2));
    ...
    // Print summary
    console.log('\n' + '='.repeat(60));
    console.log('📋 ADVANCED VISUAL i18n SCAN RESULTS');
    ...
    console.log('\n📁 Reports saved:');
    console.log('   - advanced-visual-i18n-report.json (detailed JSON)');
    console.log('   - advanced-visual-i18n-report.html (visual HTML report)');
...
<head>
    ...
    <title>Advanced Visual i18n Scan Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔍 Advanced Visual i18n Scan Report</h1>
    ...
    fs.writeFileSync('./advanced-visual-i18n-report.html', html);
  }
...
async function main() {
  ...
  const scanner = new AdvancedVisualI18nScanner(options);
...
module.exports = AdvancedVisualI18nScanner;
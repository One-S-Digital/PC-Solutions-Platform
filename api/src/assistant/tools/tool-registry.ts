...
export const TOOL_REGISTRY: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      percentage: { type: 'number', description: 'Work percentage as a number (e.g. 60 for "60%"). Extract this separately — do not embed it in contractType.' },
      location: { type: 'string', description: 'Canton or city for the position' },
      contractType: { type: 'string', description: 'Enum only — one of: FULL_TIME, PART_TIME, CDI, CDD, INTERNSHIP, REPLACEMENT, TEMPORARY, FREELANCE. No other text.' },
      startDate: { type: 'string', description: 'Optional ISO start date' },
      description: { type: 'string', description: 'Job description / details. Add any extra context from the user message that does not fit other fields.' },

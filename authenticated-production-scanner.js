...
class AuthenticatedProductionI18nScanner {
  constructor(options = {}) {
    this.browser = null;
    this.page = null;
  ...
  async init() {
    console.log('🚀 Starting Authenticated Production Visual i18n Scanner...');
  ...
  async scanPage(url, pageName, options = {}) {
    ...
    try {
      ...
      const screenshotPath = `./authenticated-i18n-screenshots/${pageName.replace(/[^a-zA-Z0-9]/g, '_')}.png`;
  ...
  async scanAllPages() {
    ...
    if (!fs.existsSync('./authenticated-i18n-screenshots')) {
      fs.mkdirSync('./authenticated-i18n-screenshots');
    }

    // First, try to authenticate
    await this.authenticate();

    // Define pages to scan - focus on authenticated pages
    const pagesToScan = [
      { url: '/', name: 'Home' },
  ...
  async generateReport() {
    console.log('\n📊 Generating Authenticated Production Visual i18n Report...');
    ...
    fs.writeFileSync('./authenticated-production-i18n-report.json', JSON.stringify(report, null, 2));
    ...
    // Print summary
    console.log('\n' + '='.repeat(60));
    console.log('📋 AUTHENTICATED PRODUCTION VISUAL i18n SCAN RESULTS');
    ...
    console.log('\n📁 Reports saved:');
    console.log('   - authenticated-production-i18n-report.json (detailed JSON)');
    console.log('   - authenticated-production-i18n-report.html (visual HTML report)');
    console.log('   - authenticated-i18n-screenshots/ (page screenshots)');
...
<head>
    ...
    <title>Authenticated Production Visual i18n Scan Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔐 Authenticated Production Visual i18n Scan Report</h1>
            ...
            <p>Authentication: ${report.authenticated ? '✅ Success' : '❌ Failed'}</p>
        </div>
    ...
    fs.writeFileSync('./authenticated-production-i18n-report.html', html);
  }
...
async function main() {
  ...
  const scanner = new AuthenticatedProductionI18nScanner(options);
  ...
  try {
    ...
    console.error('❌ Authenticated production scanner failed:', error);
  } finally {
...
module.exports = AuthenticatedProductionI18nScanner;
...
/**
 * Simple Translation Audit Script (No External Dependencies)
 * 
...
async function auditTranslations() {
  ...
  // Find frontend files
  const frontendFiles = findFiles(FRONTEND_SRC_DIR, /\.(ts|tsx|js|jsx)$/, [
    /node_modules/,
    /dist/,
    /build/,
    /coverage/,
  ...
  // Find admin files
  const adminFiles = findFiles(ADMIN_SRC_DIR, /\.(ts|tsx|js|jsx)$/, [
    /node_modules/,
    /dist/,
    /build/,
    /coverage/,
  ...
  const report = {
    ...
    summary: {
      frontend: {
        ...
        coverage: frontendCalls.length > 0 ? Math.round(((frontendCalls.length - frontendMissing.length) / frontendCalls.length) * 100) : 100
      },
      admin: {
        ...
        coverage: adminCalls.length > 0 ? Math.round(((adminCalls.length - adminMissing.length) / adminCalls.length) * 100) : 100
      }
    },
    frontend: {

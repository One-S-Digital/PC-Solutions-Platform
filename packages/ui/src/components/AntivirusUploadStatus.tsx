...
export function AntivirusUploadStatus({
  ...
  if (isScanning) {
    return (
      <div
        className={clsx(
          "flex items-center gap-3 p-4 bg-surface-2 rounded-lg",
          className,
        )}
      >
        ...
        <div>
          ...
          <p className="text-text-muted text-sm">
            {t(
              "antivirus.scanningDescription",
              "Please wait while we check your file for malware",
            )}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div>
          ...
          <p className="text-danger/80 text-sm">
            {t(
              "antivirus.infectedDescription",
              "File contains harmful code. Try exporting a clean PDF/image and re-upload.",
            )}
          </p>
        </div>
      </div>
  ...
  if (scanResult === "error") {
    return (
      <div
        className={clsx(
          "flex items-center gap-3 p-4 bg-warn/10 border border-warn/20 rounded-lg",
          className,
        )}
      >
        ...
        <div>
          <p className="text-warn font-medium">
            {t(
              "antivirus.scannerDown",
              "Virus scanner temporarily unavailable",
            )}
          </p>
          ...
          {errorMessage && (
            <p className="text-warn/70 text-xs mt-1">{errorMessage}</p>
          )}
        </div>
      </div>
    );
      ...
      >
        ...
        <div>
          ...
          <p className="text-success/80 text-sm">
            {t(
              "antivirus.cleanDescription",
              "Your file is clean and safe to use",
            )}
          </p>
        </div>
      </div>
...
export function useAntivirusUpload() {
  ...
  const [scanResult, setScanResult] = useState<
    "clean" | "infected" | "error" | undefined
  >();
  const [errorMessage, setErrorMessage] = useState<string | undefined>();

  const uploadAndScan = async (
    file: File,
  ): Promise<AntivirusUploadResult | null> => {
    setIsScanning(true);
    ...
    try {
      ...
      console.error("Antivirus upload failed", error);
      setScanResult("error");
  ...
  const checkHealth = async (): Promise<boolean> => {
    try {
      ...
      console.warn("Failed to check antivirus health", error);
      return false;
    }
  };

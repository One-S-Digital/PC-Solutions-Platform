...
export default {
  ...
  theme: {
    extend: {
      ...
      backgroundImage: {
        ...
        "top-gloss":
          "linear-gradient(to bottom, rgba(255,255,255,.6), rgba(255,255,255,0))",
      },
  ...
  plugins: [
    function ({ addVariant, addUtilities }: PluginAPI) {
      // Custom variants
      ...
      addUtilities({
        ".lift": {
          transition:
            "transform var(--dur-150) var(--ease-standard), box-shadow var(--dur-150) var(--ease-standard)",
          "&:hover": {
        ...
        ".notch-tr": {
          ...
          "&::after": {
            ...
            height: "12px",
            background:
              "linear-gradient(135deg, transparent 49%, var(--surface-1) 50%)",
          },
        },
      });
    },

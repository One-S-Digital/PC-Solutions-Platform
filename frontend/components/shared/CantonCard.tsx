  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-gray-500 mt-1">
        {count}{' '}
        {count === 1
          ? t('content:statePolicies.documentSingular', 'document')
          : t('content:statePolicies.documentPlural', 'documents')}
      </p>
      
      {/* Last Updated - hidden on mobile for space */}
      {lastUpdated && (
        <p className="text-xs text-gray-400 mt-0.5 hidden sm:block">
          {t('content:statePolicies.updatedShort', 'Updated')} {format(new Date(lastUpdated), 'MMM d')}
        </p>
      )}
    </button>
  );
